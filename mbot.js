...
userInputBox.onfocus = function(){
		document.onkeydown = function() {userInputBox.focus()}
}
...
try{
	...
	console.log("ERROR!:DB is not found or contains some errors :(, please check it.", error)
}
...
function addMessageIntoBox(name, color, message, emoji="em em-no_mouth"){
	...
	var message_composition   	  =  "<div class='message' id='{0}'>".format(message_id)
...
function speakText(text){
	...
	for(var i in text_correction){
		...
		if(search != -1)
			text = text.replace(text.substr(search,text.length-search), text_correction[i])
	}
	for(var i in text_avoid){
		if(text.includes(text_avoid[i]))
			return false
	}
...
var strManager = {
	...
	filterString: function(string){
			...
			var final_string = process_string.split(/\s+/).join(' ').trim()
			for(var special in special_characters)
				final_string = final_string.replace(special_characters[special], ' ' + special_characters[special])	
			...
			for(var tick in with_tick)
				final_string = final_string.replace(with_tick[tick], without_tick[tick])	
			return final_string
			//ej: 'hóla!' > 'hola !' 
	},
		...
		if(mark_index != -1){
			...
			for(i; i < string.length+1; i++){
				if(this.inChar(string[i], ')') || i == string.length){
					...
					var var_name_key = var_name.split(mark)[1].substr(0, var_name.length-1-mark.length)
					var string = string.replace(var_name, randomItem(replace[var_name_key]))
					return string
				}
			}
		}
	...
	strStr: function (words , string){
		...
		for(var str in string_array){
			if(this.strStrSec(words_array[str], string_array[str]))
				cout++;
		}
		return cout
	}
...
function dbSearchWord(word, indiceEspecifico=false){
	...
	for(var key_pos in dbDir){
		...
		if(answers != undefined){
			...
			for(var answer_index=0; answer_index < answers.length; answer_index++){	
	...
	if(getKeys(candidates).length > 0)
		return candidates
	else
		return false
}
...
function processReply(message){
		...
		switch(sentence_type){
			...
			case 'accion':
				switch(sentence_info){
					...
					case 'hora':
						var hora = new Date()
						reply.push([returnAnswer('respuesta-hora') + hora.getHours() + " y " + hora.getMinutes() + " minutos."])
						break;
					case 'buscar':
						var queryBusqueda = "search?q="
						var key = analisis[s_pos][0]
						var posicion = analisis[s_pos][1]
						var itemBuscar = strManager.filterString(message.replace(MBOT_DB[key][posicion], ''))
	...
	reply_template = ['desconocido', 'respuesta']
	if(!reply.length)
		reply.push(returnAnswer(reply_template))
	reply = reply.join(', ')
...
function reply(inputMessage=''){
		...
		if(userMessages!='')
			addMessageIntoBox(MBOT_VARS['user_name'], 'purple', userInputBox.value, "em em-smiley")
		else
		...
		setTimeout(function(){
						...
						addMessageIntoBox('MinBot', 'red', processReply(userMessages), "em em-robot_face")
					 }, 
					  responseDelayTime)
		coutMsj+=2;
}
